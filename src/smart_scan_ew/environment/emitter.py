...
class BaseEmitter(ABC):
    ...
    def __init__(
        self, emitter_id: int, power_dbm: float, active_from: int, active_until: int | None
    ) -> None:
...
class FixedEmitter(BaseEmitter):
    ...
    def __init__(self, cfg: FixedEmitterConfig) -> None:
        super().__init__(cfg.emitter_id, cfg.power_dbm, cfg.active_from_slot, cfg.active_until_slot)
...
class AgileEmitter(BaseEmitter):
    ...
    def __init__(self, cfg: AgileEmitterConfig, rng: np.random.Generator) -> None:
        ...
        if self.pattern == "random":
            ...
            self._hop_seq = rng.permutation(self.band_ids * (100_000 // self._n + 1)).tolist()[
                :100_000
            ]
...
class SpatiallyScanningEmitter(BaseEmitter):
    """Spatially scanning (rotating-antenna) radar emitter.

    Models a radar whose antenna rotates or scans in azimuth/elevation.
    The ES receiver can only intercept the signal during the brief window
    when the beam is pointed towards it — the *beam dwell interval*.

    Within each ``scan_period_slots``-long rotation cycle, the emitter is
    active for exactly ``round(beam_dwell_fraction * scan_period_slots)``
    consecutive slots, starting at ``phase_offset_slots`` within the cycle.

    This is the primary interception challenge described in the DRDO
    problem statement: the scheduler must predict *when* the beam will
    sweep past and dwell on that band at the right moment.
    """

    def __init__(self, cfg: SpatiallyScanningEmitterConfig) -> None:
        super().__init__(cfg.emitter_id, cfg.power_dbm, cfg.active_from_slot, cfg.active_until_slot)
        self.band_id = cfg.band_id
        self.scan_period = cfg.scan_period_slots
        self.phase = cfg.phase_offset_slots % cfg.scan_period_slots
        # Beam-on window length (minimum 1 slot)
        self._beam_on_slots = max(1, round(cfg.beam_dwell_fraction * cfg.scan_period_slots))

    def is_active(self, slot: int, rng: np.random.Generator) -> tuple[bool, int]:
        if not self._in_lifetime(slot):
            return False, self.band_id
        # Position within current scan cycle, accounting for phase offset
        pos = (slot - self.phase) % self.scan_period
        # Active only during the beam-on window at the start of each cycle
        active = pos < self._beam_on_slots
        return active, self.band_id
...
def make_emitter(cfg: EmitterConfig, rng: np.random.Generator) -> BaseEmitter:
    ...
    if isinstance(cfg, SpatiallyScanningEmitterConfig):
        return SpatiallyScanningEmitter(cfg)
