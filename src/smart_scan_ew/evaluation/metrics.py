...
def compute_metrics(episode: EpisodeSummary, reward_weights: dict | None = None) -> EpisodeMetrics:
    ...
    n_total_band_slots = n_bands * (
        obs_list[-1].time_index + obs_list[-1].dwell_slots if obs_list else 1
    )
    ...
    return EpisodeMetrics(
        ...
        median_intercept_time=float(np.median(intercept_times))
        if intercept_times
        else float("nan"),
        p95_intercept_time=float(np.percentile(intercept_times, 95))
        if intercept_times
        else float("nan"),
