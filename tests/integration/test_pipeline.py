    ...
    return ScenarioConfig(
        ...
        emitters=[
            ...
            },
            {
                ...
                "band_id": 7,
                "power_dbm": -62.0,
                "burst_duration_slots": 2,
                "inter_burst_slots": 20,
            },
        ],
    )
...
class TestFullEpisodePipeline:
    ...
    @pytest.mark.parametrize("sched_name", list(SCHEDULER_REGISTRY))
    def test_truth_never_empty_when_emitters_exist(
        self, sched_name, receiver_cfg, periodic_scenario
    ):
        sched = SCHEDULER_REGISTRY[sched_name](receiver_cfg, seed=0)
        runner = EpisodeRunner(periodic_scenario, receiver_cfg, sched, seed=0)
    ...
    def test_periodicity_aware_intercepts_periodic_emitter(self, receiver_cfg, periodic_scenario):
...
class TestBenchmarkRunner:
    def test_run_benchmark_returns_all_metrics(self, receiver_cfg, periodic_scenario, tmp_path):
        ...
        scenario_yaml.write_text(yaml.dump(periodic_scenario.model_dump()), encoding="utf-8")
